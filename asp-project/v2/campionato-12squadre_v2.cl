...
#const num_squadre = 12.
#const num_partite = (num_squadre-1)*2.
#const meta_partite = num_partite/2.
...
giornata(andata,1..11).
giornata(ritorno,12..22).

partita(andata,S1,S2,GAndata,C1):-
    squadra(S1),
    squadra(S2),
    S1!=S2,
    giornata(andata,GAndata),
    gioca_a(S1,C1).


partita(ritorno,S1,S2,GRitorno,C1):-
    squadra(S1),
    squadra(S2),
    S1!=S2,
    giornata(ritorno,GRitorno),
    gioca_a(S1,C1).
...
%% Dominio 10 squadre
% -------------------
% Ad ogni giornata di andata vengono assegnate sei partite per coppia di squadre
6  {partita(AR,S1,S2,GAndata,C1): 
    team(S1),
    team(S2),
    S1<>S2,
    giocaA(S1,C1)} 6 :- giornata(AR,GAndata).

% Ad ogni giornata di ritorno vengono assegnate sei partite per coppia di squadre
% num_partite {partitaRitorno(S1,S2,GRitorno,C1): 
%     team(S1),
%     team(S2),
%     S1<>S2,
%     giocaA(S1,C1)} num_partite :- giornata(ritorno,GRitorno).
...
% % La distanza tra una coppia di gare di andata e ritorno è di almeno 6 giornate

% :- partitaAndata(S1, S2, G1, _),partitaRitorno(S2, S1, G2, _), G2<G1+6.
